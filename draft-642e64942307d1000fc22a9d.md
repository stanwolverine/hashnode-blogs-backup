...
* **Test "fails" when the Test Function "**`panics`**"**
    ...
    You can call `panic!` macro explicitly or other code within your test function can invoke the `panic!` macro to mark a test as *"failed"*
    ...
    fn should_add_two() {
        ...
        // `panic` macro can be called explicitly
        panic!()
    
        // OR, other code or expression
        // can also call `panic` macro
        fn_which_calls_panic()
    }
    ```
...
You can use `cargo test` command to run your tests. It will run all the test functions and show the result of the tests.
...
1. `assert!` macro  
    ...
    $If the given expression doesn't evaluate to `true`, `assert!` macro will invoke `panic!` macro for you and the test will be marked as failed.
    ...
    fn pass_test() {
        let a = 1; let b = 3;
        // expression "a + b == 4" evaluates to `true`.
        // Thus, `assert!` macro will not invoke `panic!` macro
        assert!(a + b == 4);
    }
    ...
    fn fail_test() {
        let a = 2; let b = 2;
        // expression "a + b == 5" evaluates to `false`.
        // Thus, `assert!` macro will invoke `panic!` macro
        // and test will be marked as "fail"
        assert!(a + b == 5);
    }
    ```
...
2. `assert_eq!` macro  
    ...
    $If the two expressions are not equal, `assert_eq!` macro will invoke `panic!` macro for you and the test will be marked as failed.
    ...
    fn pass_test() {
        ...
        // left expression `a + b` is equal(==) to
        // right expression `4`
        // Thus, `assert_eq!` macro will not invoke `panic!` macro
        assert_eq!(a + b, 4);
    }
    ...
    fn fail_test() {
        ...
        // left expression `a + b` is not equal(==) to
        // right expression `5`
        // Thus, `assert_eq!` macro will invoke `panic!` macro
        assert_eq!(a + b, 5);
    }
    ```
...
3. `assert_ne!` macro  
    ...
    fn fail_test() {
        ...
        // left expression `a + b` is equal to
        // right expression `4`
        // Thus, `assert_ne!` macro will invoke `panic!` macro
        assert_ne!(a + b, 4);
    }
...
4. `should_panic` attribute  
    ...
    $To add the `should_panic` attribute to a function, you can simply place it on a new line after the `test` attribute. Refer to below code snippet -
    ...
    fn passed_test() {
        ...
        // expression `a + b != 4` evaluates to `false`
        // Therfore, `assert!` macro will invoke `panic!` macro
        assert!(a + b != 4);
    
        // This test will be marked as "passed" because
    ...
    fn failed_test() {
        ...
        // expression `a + b == 4` evaluates to `true`
        // Therfore, `assert!` macro will not invoke `panic!` macro
        assert!(a + b == 4);
    
        // This test will be marked as "failed" because
        // `should_panic` attribute is present, indicating that a panic is expected
        // However, in this test, the panic was not "triggered".
    }
    ```
    ...
    $If the panic happens as expected for that reason, the test will pass. However, if the panic occurs for a different reason or doesn't happen at all, the test will be marked as failed.
    ...
    fn passed_test() {
        ...
        // This test will be marked as "passed" because
        // "panic was triggered for the expected reason"
    }
    ...
    fn failed_test() {
        ...
        // This test will be marked as "failed" because
        // panic was triggered for a different reason
...
```rust
// Example 8: Passing custom error message to assertion macros
